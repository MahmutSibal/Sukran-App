...
import '../KitchenLayout/KitchenLayout.css';
import AiAssistantBubble from '../../components/AiAssistantBubble.jsx';

const menuItems = [
  ['notifications_active', 'Aktif Siparişler', '/kitchen/orders/active'],
  ['table_restaurant', 'Masalar', '/kitchen/tables'],
  ['inventory_2', 'Ürünler', '/kitchen/products'],
  ['history', 'Sipariş Geçmişi', '/kitchen/order-history'],
];
  ...
  return (
    <div className="bg-background text-on-background font-body-md antialiased overflow-x-hidden min-h-screen flex">
        ...
        <header className="bg-surface border-b border-outline-variant shadow-sm fixed top-0 right-0 w-full lg:w-[calc(100%-280px)] h-16 z-40 flex justify-between items-center px-4 md:px-gutter">
          ...
          <div className="flex items-center gap-2 md:gap-3 ml-3">
            <button
              ...
              className={`h-10 px-3 md:px-4 rounded-full border transition-all flex items-center gap-2 font-label-md text-label-sm ${
            ...
            >
              <span className="material-symbols-outlined text-[20px]">
                {isNotificationsEnabled ? 'notifications_active' : 'notifications_off'}
              </span>

              <span className="hidden md:inline">
                {isNotificationsEnabled ? 'Bildirim Açık' : 'Bildirim Kapalı'}
              </span>
            </button>
      ...
      <AiAssistantBubble
        ...
        greeting="Merhaba! Mutfak panelinde sana yardımcı olabilirim. Aktif sipariş sayısı, ürünler ve masalar hakkında güncel verilere göre soru sorabilirsin."
        systemPrompt="Mutfak personeline aktif siparişler, hazırlık önceliği, ürün durumu ve servis akışı konusunda yardımcı ol. Sipariş/ürün/sayı sorulursa yalnızca GÜNCEL VERİ bölümündeki değerleri kullan, rakam uydurma."
